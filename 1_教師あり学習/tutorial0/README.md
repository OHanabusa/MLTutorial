...
損失関数は平均2乗誤差をloss関数として定義した．

次に，backprop関数では，順伝搬を計算してから，ニューラルネットワークの更新に用いる各層の勾配計算を定義している．ここで計算している勾配の式がなぜこう書かれているかを理論の式(4)~(6)を参考に計算してみてください．

ここで，backprop関数にはすべての訓練データx_trainを渡していることに注意してください．これによって，すべての訓練データを同時に計算し，学習を行っています．

以上で，ニューラルネットワークの定義と学習に必要な関数の定義が終わったので，学習率(learning_rate)と学習回数(EPOCHS)を定めて，最急勾配降下法をもちいて学習を行う．
