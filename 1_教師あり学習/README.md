...
この際の重み行列 $\boldsymbol W^{(l)}$ やバイアスベクトル $\boldsymbol b^{(l)}$ がニューラルネットワークの学習パラメータとなる．また，第 $l$ 層目の重み行列 $\boldsymbol W^{(l)}$ とバイアスベクトル $\boldsymbol b^{(l)}$ の初期値は，一般的に，第 $l-1$ 層目のノード数 $d$ に対して， $\left(-\frac{1}{\sqrt{d}}, \frac{1}{\sqrt{d}}\right)$ の範囲でランダムに設定される．
