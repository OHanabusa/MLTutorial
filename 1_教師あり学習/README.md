...
ニューラルネットワークは入力層，出力層，そしてその間にある複数の隠れ層から構成される．この模式図を\fref{NN}に示す．図に示すように，入力層には入力ベクトル，入力ベクトルから全結合された隠れ層には隠れ層ベクトル，隠れ層ベクトルから全結合された出力層には出力層ベクトルが各層のノードの情報となる．
入力層には入力ベクトル$\bm{x}$の次元数のノードが存在し，各要素の実数を保持する．
...
I^{(l)}_i &= \sum_j W_{ij}^{(l)} h_j^{(l-1)} + b_i^{(l)} \eql{I}
\end{align}
\eref{nn}，(\ref{eq:I})より，各ノードでは，1つ前の層の全ノードからの重み付き入力の合計にバイアスを加え，活性化関数を通して，各ノードの値が計算される．
この際の重み行列$\bm{W}^{(l)}$やバイアスベクトル$\bm{b}^{(l)}$がニューラルネットワークの学習パラメータとなる．また，第$l$層目の重み行列$\bm{W}^{(l)}$とバイアスベクトル$\bm{b}^{(l)}$の初期値は，一般的に，第$l-1$層目のノード数$d$に対して，$\left(-\frac{1}{\sqrt{d}}, \frac{1}{\sqrt{d}}\right)$の範囲でランダムに設定される．
